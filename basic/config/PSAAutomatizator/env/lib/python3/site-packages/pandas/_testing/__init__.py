...
def makeTimedeltaIndex(k: int = 10, freq="D", name=None,
                       **kwargs) -> TimedeltaIndex:
    return pd.timedelta_range(start="1 day", periods=k,
                              freq=freq, name=name, **kwargs)
...
def _make_timeseries(start="2000-01-01", end="2000-12-31",
                     freq="1D", seed=None):
    """
...
def makePeriodSeries(nper=None, name=None):
    if nper is None:
        nper = _N
    return Series(np.random.randn(nper),
                  index=makePeriodIndex(nper), name=name)
...
def makeCustomDataframe(
    ...
    assert r_idx_type is None or (
        r_idx_type in ("i", "f", "s", "u", "dt", "p",
                       "td") and r_idx_nlevels == 1
    )
    assert c_idx_type is None or (
        c_idx_type in ("i", "f", "s", "u", "dt", "p",
                       "td") and c_idx_nlevels == 1
    )
...
def test_parallel(num_threads=2, kwargs_list=None):
    ...
    def wrapper(func):
        ...
        def inner(*args, **kwargs):
            if has_kwargs_list:
                def update_kwargs(i): return dict(kwargs, **kwargs_list[i])
            else:
                def update_kwargs(i): return kwargs
            threads = []
...
def shares_memory(left, right) -> bool:
    ...
    if isinstance(left, ExtensionArray) and left.dtype == "string[pyarrow]":
        ...
        if isinstance(
                right, ExtensionArray) and right.dtype == "string[pyarrow]":
