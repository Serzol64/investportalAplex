...
class TestStyler:
    ...
    def test_apply_series_return(self, axis):
        ...
        # labels OK
        def func(s): return pd.Series(["color: red;"], index=["Y"])
    ...
    def test_bad_apply_shape(self):
        ...
        with pytest.raises(ValueError, match=msg.format("index")):
            df.style._apply(lambda x: pd.Series(
    ...
    def test_hide_single_index(self):
        ...
        ctx4 = self.df.set_index("A").style.hide(
    ...
    def test_hide_columns_single_level(self):
        ...
        ctx = self.df.style.hide(
            ["A", "B"], axis="columns")._translate(True, True)
        assert not ctx["head"][0][1]["is_visible"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # allow missing
            pd.IndexSlice[pd.IndexSlice[:, "W", ["Y", "Z", "-"]], :],
            pd.IndexSlice[pd.IndexSlice["U", "W",
                                        ["Y", "Z", "-"]], :],  # no slice
            # check simultaneous
            pd.IndexSlice[pd.IndexSlice[:, "W", "Y"],
            ...
            if isinstance(key, tuple):
                for subkey in key:
                    if isinstance(subkey, list) and "-" in subkey:
                        # not present in the index level, ignored, will raise
                        # in future
                        warn = FutureWarning
