...
def _get_layout(nplots: int, layout=None,
...
def _has_externally_shared_axis(
        ax1: matplotlib.axes, compare_axis: str) -> bool:
    """
...
def handle_shared_axes(
    ...
    if nplots > 1:
        if compat.mpl_ge_3_2_0():
            ...
            def row_num(x): return x.rowNum
            def col_num(x): return x.colNum

        if compat.mpl_ge_3_4_0():
            def is_first_col(x): return x.get_subplotspec().is_first_col()
        else:
            def is_first_col(x): return x.is_first_col()
        ...
        if nrows > 1:
            try:
                ...
                if compat.mpl_ge_3_4_0():
                    def is_last_row(
                        x): return x.get_subplotspec().is_last_row()
                else:
                    def is_last_row(x): return x.is_last_row()
                for ax in axarr:
                    if is_last_row(ax):
                        continue
                    if sharex or _has_externally_shared_axis(ax, "x"):
