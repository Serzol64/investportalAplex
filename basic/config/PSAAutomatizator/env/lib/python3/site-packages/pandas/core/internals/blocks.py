...
class Block(PandasObject):
    ...
    @final
    def _maybe_downcast(
            self, blocks: list[Block], downcast=None) -> list[Block]:
    ...
    @final
    def astype(self, dtype: DtypeObj, copy: bool = False,
               errors: str = "raise"):
        """
    ...
    def replace_list(
        ...
        # ndarray]"
        masks = [extract_bool_array(x)
                 for x in masks]  # type: ignore[arg-type]
        ...
        for i, (src, dest) in enumerate(pairs):
            ...
            for blk_num, blk in enumerate(rb):
                ...
                if convert and blk.is_object and not all(
                        x is None for x in dest_list):
    ...
    def setitem(self, indexer, value):
        ...
        if is_empty_indexer(indexer, arr_value):
            ...
            value = setitem_datetimelike_compat(
                values, len(values[indexer]), value)
            values[indexer] = value
    ...
    def shift(self, periods: int, axis: int = 0,
              fill_value: Any = None) -> list[Block]:
    ...
    def where(self, other, cond) -> list[Block]:
        ...
        if is_valid_na_for_dtype(
                other, self.dtype) and self.dtype != _dtype_obj:
            other = self.fill_value
...
class NDArrayBackedExtensionBlock(
        libinternals.NDArrayBackedBlock, EABackedBlock):
    """
