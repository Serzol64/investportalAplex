...
def concat_arrays(to_concat: list) -> ArrayLike:
    ...
    if target_dtype.kind in ["m", "M"]:
        ...
        to_concat = [
            arr.to_array(target_dtype) if isinstance(
                arr, NullArrayProxy) else arr
...
def concatenate_managers(
    ...
    if isinstance(mgrs_indexers[0][0], ArrayManager):
        return _concatenate_array_managers(
            mgrs_indexers, axes, concat_axis, copy)
    ...
    concat_plans = [
        _get_mgr_concatenation_plan(mgr) for mgr,
        _ in mgrs_indexers]
...
def make_na_array(dtype: DtypeObj, shape: Shape) -> ArrayLike:
    ...
    if is_datetime64tz_dtype(dtype):
        ...
        return missing_arr.take(taker, allow_fill=True,
...
def _concatenate_join_units(
    ...
    to_concat = [
        ju.get_reindexed_values(
            empty_dtype=empty_dtype) for ju in join_units]
    ...
    if len(to_concat) == 1:
        ...
        to_concat = [
            ...
            t if is_1d_only_ea_obj(t) else t[0, :]
            for t in to_concat
        ]
...
def _is_uniform_join_units(join_units: list[JoinUnit]) -> bool:
    ...
    return (
        ...
        # Int64+int64
        all(isinstance(ju.block, type(first)) for ju in join_units)
        and
...
def _combine_concat_plans(plans):
    ...
    if len(plans) == 1:
        ...
        # singleton list so we can modify it as a side-effect within
        # _next_or_none
        num_ended = [0]
