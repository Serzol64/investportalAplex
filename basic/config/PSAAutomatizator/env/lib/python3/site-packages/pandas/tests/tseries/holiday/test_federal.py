...
def test_no_mlk_before_1986():
    ...
    holidays = MLKCalendar().holidays(
        start="1984", end="1988").to_pydatetime().tolist()

    # Testing to make sure holiday is not incorrectly observed before 1986.
    assert holidays == [
        datetime(
            1986, 1, 20, 0, 0), datetime(
            1987, 1, 19, 0, 0)]
...
def test_memorial_day():
    ...
    holidays = MemorialDay().holidays(
        start="1971", end="1980").to_pydatetime().tolist()
