    ...
    def aggregate(self, func=None, *args, engine=None,
        ...
        if maybe_use_numba(engine):
            ...
            return self.obj._constructor(
                result.ravel(), index=index, name=data.name)
    ...
    def _wrap_applied_output(
        ...
        if isinstance(values[0], dict):
            ...
            return self._concat_objects(
                values, not_indexed_same=not_indexed_same)
        else:
            # GH #6265 #24880
...
class DataFrameGroupBy(GroupBy[DataFrame]):
    ...
    def aggregate(self, func=None, *args, engine=None,
        ...
        if maybe_use_numba(engine):
            ...
            index = self.grouper.result_index
            return self.obj._constructor(
                result, index=index, columns=data.columns)
                ...
                try:
                    ...
                    if isinstance(sobj, Series):
                        ...
                        result.columns._set_names(
                            sobj.columns.names, level=list(
                                range(sobj.columns.nlevels))
                        )
    ...
    def _aggregate_frame(self, func, *args, **kwargs) -> DataFrame:
        ...
        if self.axis == 0:
            # test_pass_args_kwargs_duplicate_columns gets here with non-unique
            # columns
    ...
    def _apply_to_column_groupbys(
            self, func, obj: DataFrame | Series) -> DataFrame:
        from pandas.core.reshape.concat import concat
    ...
    def idxmax(self, axis=0, skipna: bool = True):
        ...
        def func(df):
            # NB: here we use numeric_only=None, in DataFrame it is False
            # GH#38217
            res = df._reduce(
                nanops.nanargmax,
                "argmax",
    ...
    def value_counts(
        ...
        with self._group_selection_context():
            ...
            if normalize:
                ...
                levels = list(
                    range(len(self.grouper.groupings), result.index.nlevels))
    ...
    if isinstance(res, Series):
        ...
        if res.index.is_(obj.index):
            ...
            res_frame = obj._constructor(
                np.concatenate([res.values] *
                               len(group.index)).reshape(group.shape),
