...
class BaseSetitemTests(BaseExtensionTests):
    ...
    def test_setitem_frame_2d_values(self, data, request):
        ...
        if using_array_manager:
            if not isinstance(
                data.dtype, (PandasDtype, PeriodDtype,
                             IntervalDtype, DatetimeTZDtype)
            ):
                # These dtypes have non-broken implementations of
                # _can_hold_element
                mark = pytest.mark.xfail(
                    reason="Goes through split path, loses dtype")
