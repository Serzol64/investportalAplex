    ...
    def _maybe_add_join_keys(
        ...
        for i, (name, lname, rname) in enumerate(keys):
            ...
            if take_left is not None or take_right is not None:
                ...
                # error: Item "bool" of "Union[Any, bool]" has no attribute
                # "all"
        ...
        if self.left_index and self.right_index and self.how != "asof":
            ...
            if self.right_index:
                if len(self.left) > 0:
                    ...
                    left_indexer = np.array(
                        [-1] * len(join_index), dtype=np.intp)
...
class _AsOfMerge(_OrderedMerge):
        ...
        for i, (lk, rk) in enumerate(zip(left_join_keys, right_join_keys)):
            if not is_dtype_equal(lk.dtype, rk.dtype):
                if is_categorical_dtype(
                        lk.dtype) and is_categorical_dtype(rk.dtype):
    ...
    def _get_join_indexers(
        ...
        if self.left_by is not None:
            ...
            # error: Cannot call function of unknown type
            left_by_values = by_type_caster(
                left_by_values)  # type: ignore[operator]
            # error: Cannot call function of unknown type
            right_by_values = by_type_caster(
                right_by_values)  # type: ignore[operator]
            ...
            return func(left_values, right_values,
                        self.allow_exact_matches, tolerance)
