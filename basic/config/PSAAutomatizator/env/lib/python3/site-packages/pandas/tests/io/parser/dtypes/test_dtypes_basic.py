...
def test_decimal_and_exponential(
        python_parser_only, numeric_decimal, thousands):
    # GH#31920
...
def test_1000_sep_decimal_float_precision(
    ...
    if isinstance(value, str):  # the negative cases (parse as text)
        value = " " + value + " "
    decimal_number_check(c_parser_only, (text, value),
                         thousands, float_precision)
...
def decimal_number_check(parser, numeric_decimal, thousands, float_precision):
    ...
    if thousands is None and "_" in value:
        pytest.skip(
            "Skip test if no thousands sep is defined and sep is in value")
    df = parser.read_csv(
        StringIO(value),
        sep="|",
        thousands=thousands,
