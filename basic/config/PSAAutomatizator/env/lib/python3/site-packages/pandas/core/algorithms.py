...
def isin(comps: AnyArrayLike, values: AnyArrayLike) -> npt.NDArray[np.bool_]:
    ...
    if len(comps) > 1_000_000 and len(
            values) <= 26 and not is_object_dtype(comps):
...
def factorize(
    ...
    if isinstance(original, ABCIndex):
        if original.dtype.kind in [
                "m", "M"] and isinstance(uniques, np.ndarray):
            original._data = cast(
                "Union[DatetimeArray, TimedeltaArray]", original._data
            )
            uniques = type(
                original._data)._simple_new(
                uniques,
                dtype=original.dtype)
        uniques = original._shallow_copy(uniques, name=None)
    elif isinstance(original, ABCSeries):
    ...
    def compute(self, method: str) -> Series:
        ...
        if is_extension_array_dtype(dropped.dtype):
            ...
            if isinstance(arr, BaseMaskedArray):
                ...
                result = type(self)(
                    ser, n=self.n, keep=self.keep).compute(method)
