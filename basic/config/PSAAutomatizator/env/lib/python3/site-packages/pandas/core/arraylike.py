...
def _maybe_fallback(ufunc: np.ufunc, method: str, *inputs: Any, **kwargs: Any):
    ...
    if n_alignable >= 2 and n_frames >= 1:
        ...
        # the first frame is what determines the output index/columns in pandas
        # < 1.2
        ...
        # if at least one is not aligned -> warn and fallback to array
        # behaviour
        if non_aligned:
            warnings.warn(
...
def array_ufunc(self, ufunc: np.ufunc, method: str,
    ...
    # for backwards compatibility check and potentially fallback for
    # non-aligned frames
