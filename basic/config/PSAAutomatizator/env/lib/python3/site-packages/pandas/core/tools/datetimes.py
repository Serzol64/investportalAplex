...
def _return_parsed_timezone_results(
        result: np.ndarray, timezones, tz, name) -> Index:
    """
    ...
    if tz is not None:
        ...
        tz_results = np.array([tz_result.tz_convert(tz)
...
def _array_strptime_with_fallback(
    ...
    try:
        result, timezones = array_strptime(
            arg, fmt, exact=exact, errors=errors)
...
def _attempt_YYYYMMDD(arg: np.ndarray, errors: str) -> np.ndarray | None:
    ...
    try:
        ...
        mask = ~algorithms.isin(
            arg, list(nat_strings))  # type: ignore[arg-type]
