...
class BaseMaskedArray(OpsMixin, ExtensionArray):
    ...
    def __init__(self, values: np.ndarray,
                 mask: np.ndarray, copy: bool = False):
    ...
    @overload
    def __getitem__(self: BaseMaskedArrayT,
                    item: SequenceIndexer) -> BaseMaskedArrayT:
        ...
    ...
    def factorize(self, na_sentinel: int = -
                  1) -> tuple[np.ndarray, ExtensionArray]:
        arr = self._data
        mask = self._mask

        codes, uniques = factorize_array(
            arr, na_sentinel=na_sentinel, mask=mask)
    ...
    def _wrap_reduction_result(self, name: str, result, skipna, **kwargs):
        if isinstance(result, np.ndarray):
            ...
            return self._maybe_mask_result(
                result, mask, other=None, op_name=name)
        return result
    ...
    def prod(self, *, skipna=True, min_count=0,
             axis: int | None = 0, **kwargs):
