...
def test_array_equivalent_str():
    for dtype in ["O", "S", "U"]:
        assert array_equivalent(
            np.array(["A", "B"], dtype=dtype), np.array(
                ["A", "B"], dtype=dtype)
        )
        assert not array_equivalent(
            np.array(["A", "B"], dtype=dtype), np.array(
...
def test_array_equivalent_nested():
    ...
    left = np.array(
        [np.array([50, 70, 90]), np.array([20, 30, 40])], dtype=object)
    right = np.array(
        [np.array([50, 70, 90]), np.array([20, 30, 40])], dtype=object)
    ...
    left = np.array(
        [np.array([50, 50, 50]), np.array([40, 40, 40])], dtype=object)
...
def test_na_value_for_dtype(dtype, na_value):
    ...
    assert result is na_value or (
        isna(result) and isna(na_value) and isinstance(result, type(na_value))
    )
