...
class Index(IndexOpsMixin, PandasObject):
    ...
    @final
    def _left_indexer_unique(
            self: _IndexT, other: _IndexT) -> npt.NDArray[np.intp]:
    ...
    @classmethod
    def _simple_new(cls: type[_IndexT], values,
    ...
    def _with_infer(cls, *args, **kwargs):
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings(
                "ignore", ".*the Index constructor", FutureWarning)
    ...
    def _shallow_copy(self: _IndexT, values,
    ...
    def astype(self, dtype, copy: bool = True):
        ...
        return Index(new_values, name=self.name,
    ...
    def drop_duplicates(self: _IndexT, keep: str_t |
                        bool = "first") -> _IndexT:
        """
    ...
    def union(self, other, sort=None):
        ...
        if not is_dtype_equal(self.dtype, other.dtype):
            ...
            # or after the is_dtype_equal check above affects the returned
            # dtype
    ...
    def _wrap_setop_result(self, other: Index, result) -> Index:
        ...
        if isinstance(self, Index) and self.dtype != _dtype_obj:
    ...
    def join(
        ...
        if not self.is_unique and not other.is_unique:
            ...
            # not handle this in case of not lexicographically monotonic
            # GH#38502
            try:
                return self._join_monotonic(other, how=how)
            except TypeError:
    ...
    def _get_engine_target(self) -> np.ndarray:
        ...
        if isinstance(self, Index) and isinstance(
                self._values, ExtensionArray):
    ...
    def asof_locs(self, where: Index,
                  mask: np.ndarray) -> npt.NDArray[np.intp]:
        """
    ...
    def get_indexer_non_unique(
        ...
        if not self._should_compare(
                target) and not is_interval_dtype(self.dtype):
            # IntervalIndex get special treatment bc numeric scalars can be
            #  matched to Interval scalars
            return self._get_indexer_non_comparable(
    ...
    def _find_common_type_compat(self, target) -> DtypeObj:
        ...
        special cases.
        """
        if is_interval_dtype(self.dtype) and is_valid_na_for_dtype(
                target, self.dtype):
    ...
    def _transform_index(self, func, *, level=None) -> Index:
        ...
        if isinstance(self, ABCMultiIndex):
            if level is not None:
                ...
                items = [
                    tuple(
                        func(y) if i == level else y for i,
                        y in enumerate(x))
                    for x in self
                ]
            else:
    ...
    def _arith_method(self, other, op):
        if (
            ...
            and not isinstance(other, Index)
        ):
...
def ensure_index(index_like: AnyArrayLike | Sequence,
    ...
    if isinstance(index_like, list):
        if not isinstance(index_like, list):
