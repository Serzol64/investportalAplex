...
# Until https://github.com/numpy/numpy/issues/19078 is sorted out, just
# suppress
    ...
    for item in items:
        ...
        for (mark, kwd, skip_if_found, arg_name) in marks:
            if kwd in item.keywords:
                ...
                if skip_if_found:
                    item.add_marker(
                        pytest.mark.skip(f"skipping due to {arg_name}"))
                    break
...
# error: List item 0 has incompatible type "Type[Index]"; expected
# "Type[IndexOpsMixin]"
@pytest.fixture(
