...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def _set_group_selection(self) -> None:
        ...
        if len(groupers):
            ...
            self._group_selection = ax.difference(
                Index(groupers), sort=False).tolist()
    ...
    def _wrap_applied_output(self, data, values: list,
                             not_indexed_same: bool = False):
        raise AbstractMethodError(self)

    def _resolve_numeric_only(self, numeric_only: bool |
                              lib.NoDefault) -> bool:
        """
        ...
        if numeric_only is lib.no_default:
            ...
            if self.obj.ndim == 2:
                ...
                # Regression from 1.2.5 to 1.3 caused object columns to be
                # dropped
                if self.axis:
                ...
                if len(obj.columns) and not len(
                        check.columns) and not obj.empty:
                    numeric_only = False
    ...
    @final
    def _transform_with_numba(self, data, func, *args,
    ...
    @final
    def _aggregate_with_numba(self, data, func, *args,
                              engine_kwargs=None, **kwargs):
        """
    ...
    def _python_agg_general(self, func, *args, **kwargs):
        ...
        for idx, obj in enumerate(self._iterate_slices()):
            ...
            try:
                # if this function is invalid for this dtype, we will ignore
                # it.
    ...
    def _bool_agg(self, val_test: Literal["any", "all"], skipna: bool):
        ...
        def result_to_bool(
            ...
            if nullable:
                return BooleanArray(result.astype(
                    bool, copy=False), result == -1)
            else:
    ...
    def count(self) -> Series | DataFrame:
        ...
        def hfunc(bvalues: ArrayLike) -> ArrayLike:
            ...
            counted = lib.count_level_2d(
                masked, labels=ids, max_bin=ngroups, axis=1)
            if is_series:
        ...
        if maybe_use_numba(engine):
            ...
            return np.sqrt(
                self._numba_agg_general(
                    sliding_var, engine_kwargs, "groupby_std", ddof)
            )
        else:
            return self._get_cythonized_result(
    ...
    def var(
        ...
        if maybe_use_numba(engine):
            ...
            if ddof == 1:
                ...
                def func(x): return x.var(ddof=ddof)
                with self._group_selection_context():
    ...
    def sem(self, ddof: int = 1):
        ...
        if result.ndim == 1:
            ...
            result.iloc[:,
                        result_ilocs] /= np.sqrt(counts.iloc[:,
                                                             count_ilocs])
        return result
    ...
    def quantile(self, q=0.5, interpolation: str = "linear"):
        ...
        def pre_processor(
                vals: ArrayLike) -> tuple[np.ndarray, np.dtype | None]:
        ...
        def post_processor(vals: np.ndarray,
                           inference: np.dtype | None) -> np.ndarray:
            if inference:
                # Check for edge case
                if not (
    ...
    def rank(
        ...
        if axis != 0:
            ...
            return self.apply(lambda x: x.rank(
                axis=axis, numeric_only=False, **kwargs))
    ...
    def cummin(self, axis=0, **kwargs):
        ...
        return self._cython_transform(
            "cummin", numeric_only=False, skipna=skipna)
    ...
    def cummax(self, axis=0, **kwargs):
        ...
        return self._cython_transform(
            "cummax", numeric_only=False, skipna=skipna)
    ...
    def shift(self, periods=1, freq=None, axis=0, fill_value=None):
        ...
        if freq is not None or axis != 0:
            return self.apply(lambda x: x.shift(
    ...
    @Substitution(name="groupby")
    @Appender(_common_see_also)
    def pct_change(self, periods=1, fill_method="ffill",
                   limit=None, freq=None, axis=0):
        """
    ...
    def _reindex_output(
        ...
        index, _ = MultiIndex.from_product(
            levels_list, names=names).sortlevel()
