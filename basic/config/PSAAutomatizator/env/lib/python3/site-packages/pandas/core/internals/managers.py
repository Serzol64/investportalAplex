...
class BaseBlockManager(DataManager):
    ...
    @classmethod
    def from_blocks(cls: type_t[T], blocks: list[Block],
                    axes: list[Index]) -> T:
        raise NotImplementedError
    ...
    def shift(self: T, periods: int, axis: int, fill_value) -> T:
        ...
        return self.apply("shift", periods=periods,
    ...
    def _make_na_block(
        ...
        # Tuple[Any, Any]]"
        block_values = np.empty(block_shape,
...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
    ...
    def __init__(
        ...
        if verify_integrity:
            ...
            for block in blocks:
                ...
                if isinstance(
                        block, DatetimeTZBlock) and block.values.ndim == 1:
    ...
    def grouped_reduce(self: T, func: Callable,
        ...
        if ignore_failures:
            if res_blocks:
                indexer = np.concatenate(
                    [blk.mgr_locs.as_array for blk in res_blocks])
    ...
    def to_dict(self, copy: bool = True):
        ...
        return {dtype: self._combine(blocks, copy=copy)
...
class SingleBlockManager(BaseBlockManager, SingleDataManager):
    ...
    @classmethod
    def from_blocks(cls, blocks: list[Block],
                    axes: list[Index]) -> SingleBlockManager:
        """
        Constructor for BlockManager and SingleBlockManager with same signature.
        """
        assert len(blocks) == 1
...
def _tuples_to_blocks_no_consolidate(tuples) -> list[Block]:
    ...
    return [
        new_block_2d(
            ensure_block_shape(
                x[1], ndim=2), placement=BlockPlacement(
                x[0]))
        for x in tuples
    ]
