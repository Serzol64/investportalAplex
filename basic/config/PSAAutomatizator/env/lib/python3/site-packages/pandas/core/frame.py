...
class DataFrame(NDFrame, OpsMixin):
    ...
    def dot(self, other: AnyArrayLike | DataFrame) -> DataFrame | Series:
        ...
        if isinstance(other, DataFrame):
            ...
            return self._constructor_sliced(
                np.dot(lvals, rvals), index=left.index)
        ...
        **Non-square DataFrame with mixed dtypes**
        ...
        if self._can_fast_transpose:
            ...
            result = self._constructor(
                new_vals, index=self.columns, columns=self.index)

        elif (
            self._is_homogeneous_type and dtypes and is_extension_array_dtype(
                dtypes[0])
        ):
            # We have EAs with the same dtype. We can preserve that dtype in
            # transpose.
            dtype = dtypes[0]
            ...
            result = self._constructor(
                new_arr, index=self.columns, columns=self.index)
        ...
        * To select Pandas datetimetz dtypes, use ``'datetimetz'`` (new in
        ...
        def dtype_predicate(dtype: DtypeObj, dtypes_set) -> bool:
            return issubclass(dtype.type, tuple(dtypes_set)) or (
                np.number in dtypes_set and getattr(
                    dtype, "_is_numeric", False)
            )
    ...
    def _reindex_axes(self, axes, level, limit, tolerance,
    ...
    def reset_index(
        ...
        if not drop:
            ...
            if isinstance(self.index, MultiIndex):
                ...
                names = [default] if self.index.name is None else [
                    self.index.name]
    ...
    def nlargest(self, n: int, columns: IndexLabel,
    ...
    def nsmallest(self, n: int, columns: IndexLabel,
                  keep: str = "first") -> DataFrame:
        """
    ...
    def _dispatch_frame_op(self, right, func: Callable,
                           axis: int | None = None):
        """
...
"""
    ...
    def explode(
        ...
        if len(columns) == 1:
            ...
            def mylen(x): return len(x) if is_list_like(x) else -1
    ...
    def cov(self, min_periods: int | None = None,
    ...
    def corrwith(self, other, axis: Axis = 0, drop=False,
    ...
    def _count_level(self, level: Level, axis: int = 0,
        ...
        counts = lib.count_level_2d(
            mask, level_codes, len(level_index), axis=axis)

        if axis == 1:
            result = self._constructor(
                counts, index=agg_axis, columns=level_index)
        else:
            result = self._constructor(
                counts, index=level_index, columns=agg_axis)
        ...
        **kwds,
        ...
        if numeric_only is None and name in ["mean", "median"]:
            ...
            if dtype_is_dt.any():
                ...
                #  self = self[cols]
                def predicate(x): return not is_datetime64_any_dtype(x.dtype)
                mgr = self._mgr._get_data_subset(predicate)
                self = type(self)(mgr)
        ...
        if hasattr(result, "dtype"):
            if filter_type == "bool" and notna(result).all():
                ...
                try:
                    ...
                    # try to coerce to the original dtypes item by item if we
                    # can
                    pass
    ...
    def quantile(
        ...
        if not is_list_like(q):
            # BlockManager.quantile expects listlike, so we wrap and unwrap
            # here
            res = self.quantile(
        ...
        if len(data.columns) == 0:
            ...
            return self._constructor_sliced(
                [], index=cols, name=q, dtype=np.float64)
