...
def _series_and_2d_ndarray(lhs, rhs):
    return (
        isinstance(
            lhs, Series) and isinstance(
            rhs, np.ndarray) and rhs.ndim > 1
...
def _is_py3_complex_incompat(result, expected):
    return isinstance(expected, (complex, np.complexfloating)
                      ) and np.isnan(result)


_good_arith_ops = sorted(
    set(ARITH_OPS_SYMS).difference(SPECIAL_CASE_ARITH_OPS_SYMS))
...
class TestAlignment:
    ...
    def test_medium_complex_frame_alignment(
            self, engine, parser, r1, c1, r2, c2):
