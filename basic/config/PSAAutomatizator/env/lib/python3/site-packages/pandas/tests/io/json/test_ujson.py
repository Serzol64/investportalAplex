...
def get_int32_compat_dtype(numpy, orient):
    # See GH#32527
    dtype = np.int64
    if not ((numpy is None or orient == "index")
            or (numpy is True and orient is None)):
...
class TestUltraJSONTests:
        ...
        [
            ...
            pytest.param(
                ...
                marks=pytest.mark.xfail(
                    PY310, reason="Failing on Python 3.10 GH41940"),
            ),
            pytest.param(
                None,
                marks=pytest.mark.xfail(
    ...
    def test_encode_list_long_conversion(self):
        ...
        tm.assert_numpy_array_equal(
            np.array(long_input), ujson.decode(
                output, numpy=True, dtype=np.int64)
        )
