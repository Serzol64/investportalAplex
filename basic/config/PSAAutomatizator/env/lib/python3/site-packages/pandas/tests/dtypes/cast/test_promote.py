...
def test_maybe_promote_datetime64_with_any(
        datetime64_dtype, any_numpy_dtype_reduced):
...
def test_maybe_promote_any_with_datetime64(
    ...
    if isinstance(fill_value, datetime.date) and dtype.kind == "M":
        # Casting date to dt64 is deprecated
        warn = FutureWarning

    with tm.assert_produces_warning(warn, match=msg):
        # stacklevel is chosen to make sense when called from higher-level
        # functions
        _check_promote(dtype, fill_value, expected_dtype, exp_val_for_scalar)
...
def test_maybe_promote_timedelta64_with_any(
        timedelta64_dtype, any_numpy_dtype_reduced):
...
def test_maybe_promote_any_numpy_dtype_with_na(
        any_numpy_dtype_reduced, nulls_fixture):
