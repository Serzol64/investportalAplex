...
def maybe_cast_to_extension_array(
    ...
    # Everything can be converted to StringArrays, but we may not want to
    # convert
...
def _maybe_promote(dtype: np.dtype, fill_value=np.nan):
    ...
    if issubclass(dtype.type, np.datetime64):
        ...
        if isinstance(fill_value, date) and not isinstance(
                fill_value, datetime):
...
def infer_dtype_from_scalar(
        val, pandas_dtype: bool = False) -> tuple[DtypeObj, Any]:
    """
...
def maybe_upcast(
    ...
    # We get a copy in all cases _except_ (values.dtype == new_dtype and not
    # copy)
...
def astype_dt64_to_dt64tz(
    ...
    if via_utc:
        ...
        raise NotImplementedError(
            "dtype_equal case should be handled elsewhere")
...
def soft_convert_objects(
    ...
    if numeric and is_object_dtype(values.dtype):
        converted, _ = lib.maybe_convert_numeric(
...
def maybe_cast_to_datetime(
                ...
                try:
                    if is_datetime64:
                        ...
                        if dta.tz is not None:
                            ...
                            # equiv: Series(dta).astype(dtype) # though
                            # deprecated
    ...
    # at this point we have converted or raised in all cases where we had a
    # list
    return cast(ArrayLike, value)


def sanitize_to_nanoseconds(values: np.ndarray,
...
def _validate_numeric_casting(dtype: np.dtype, value: Scalar) -> None:
    ...
    if (
        ...
        if is_scalar(value) and not is_bool(value):
            raise ValueError(
                f"Cannot assign {type(value).__name__} to bool series")
