...
def assert_stat_op_calc(
    ...
    if check_dates:
        expected_warning = FutureWarning if opname in [
            "mean", "median"] else None
...
def assert_stat_op_api(opname, float_frame,
...
class TestDataFrameReductions:
    ...
    def test_reductions_skipna_none_raises(
        ...
        if reduction_functions in ["count", "mad"]:
            pytest.skip(
                "Count does not accept skipna. Mad needs a deprecation cycle.")
        obj = frame_or_series([1, 2, 3])
...
def test_mad_nullable_integer_all_na(any_signed_int_ea_dtype):
    ...
    df2.iloc[:, 1] = pd.array([pd.NA] *
                              len(df2), dtype=any_signed_int_ea_dtype)
    result = df2.mad()
    expected = df.mad()
    expected[1] = pd.NA
    tm.assert_series_equal(result, expected)
