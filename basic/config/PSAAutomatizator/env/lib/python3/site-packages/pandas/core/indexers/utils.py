...
def is_list_like_indexer(key) -> bool:
    ...
    return is_list_like(key) and not (isinstance(
        key, tuple) and not isinstance(key, tuple))
...
def is_empty_indexer(indexer, arr_value: ArrayLike) -> bool:
    ...
    if arr_value.ndim == 1:
        if not isinstance(indexer, tuple):
            indexer = (indexer,)
        return any(isinstance(idx, np.ndarray)
                   and len(idx) == 0 for idx in indexer)
    return False
...
def check_setitem_lengths(indexer, value, values) -> bool:
    ...
    if isinstance(indexer, (np.ndarray, list)):
        ...
        if is_list_like(value):
            if len(value) != length_of_indexer(
                    indexer, values) and values.ndim == 1:
                # In case of two dimensional value is used row-wise and
                # broadcasted
                raise ValueError(
