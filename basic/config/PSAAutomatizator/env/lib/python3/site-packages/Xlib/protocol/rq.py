...
class String8(ValueField):
    ...
    def pack_value(self, val):
        ...
        if _PY3 and isinstance(val, str):
...
class Object(ValueField):
    ...
    def check_value(self, val):
        ...
        if isinstance(val, dict):
            ...
            raise TypeError(
                'Object value must be tuple, dictionary or DictWrapper: %s' %
                val)
        ...
        for field, flag in self.fields:
            if mask & flag:
                if field.structcode:
                    ...
                    vals, d = field.parse_binary_value(
                        data[:4], display, None, None)
...
class ModifierMapping(ValueField):
    ...
    def pack_value(self, value):
        if len(value) != 8:
            raise BadDataError(
                'ModifierMapping list should have eight elements')
...
class EventField(ValueField):
    ...
    def parse_binary_value(self, data, display, length, format):
        ...
        estruct = display.event_classes.get(
            _bytes_item(data[0]) & 0x7f, event.AnyEvent)
...
class ReplyRequest(GetAttrData):
    ...
    def _parse_response(self, data):
        self._response_lock.acquire()
        self._data, d = self._reply.parse_binary(
            data, self._display, rawdict=1)
        self._response_lock.release()
    ...
    def __repr__(self):
        return '<%s serial = %s, data = %s, error = %s>' % (
            self.__class__, self._serial, self._data, self._error)
