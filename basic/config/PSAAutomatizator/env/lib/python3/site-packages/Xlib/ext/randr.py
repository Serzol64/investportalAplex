...
def _1_0set_screen_config(self, size_id, rotation,
                          config_timestamp, timestamp=X.CurrentTime):
...
def set_screen_config(self, size_id, rotation,
                      config_timestamp, rate=0, timestamp=X.CurrentTime):
...
class GetScreenInfo(rq.ReplyRequest):
    ...
    _reply = rq.Struct(
        ...
        rq.Card16('rotation'),
        rq.Card16('rate'),  # added in version 1.1
        # XCB's protocol description disagrees with the X headers on this;
        # ignoring.
        rq.Card16('n_rate_ents'),
        rq.Pad(2),
        rq.List('sizes', RandR_ScreenSizes),
        # rq.List('rates', RandR_Rates) #FIXME: Why does uncommenting this
        # cause an error?
    )
...
def set_screen_size(self, width, height,
                    width_in_millimeters=None, height_in_millimeters=None):
...
def set_crtc_config(self, crtc, config_timestamp, mode,
                    rotation, timestamp=X.CurrentTime):
...
class SetCrtcTransform(rq.Request):
    _request = rq.Struct(
        ...
        # FIXME: The protocol says FIXED?
        # http://cgit.freedesktop.org/xorg/proto/randrproto/tree/randrproto.txt#n2161
...
def set_panning(self, crtc, left, top, width, height, track_left, track_top, track_width,
                track_height, border_left, border_top, border_width, border_height, timestamp=X.CurrentTime):
    return SetPanning(
        display=self.display,
        opcode=self.display.get_extension_major(extname),
        crtc=crtc,
...
def init(disp, info):
    ...
    disp.extension_add_method(
        'display',
        'xrandr_get_output_info',
        get_output_info)
    disp.extension_add_method(
        'display',
        'xrandr_list_output_properties',
        list_output_properties)
    disp.extension_add_method(
        'display',
        'xrandr_query_output_property',
        query_output_property)
    disp.extension_add_method(
        'display',
        'xrandr_configure_output_property ',
        configure_output_property)
    disp.extension_add_method(
        'display',
        'xrandr_change_output_property',
        change_output_property)
    disp.extension_add_method(
        'display',
        'xrandr_delete_output_property',
        delete_output_property)
    disp.extension_add_method(
        'display',
        'xrandr_get_output_property',
        get_output_property)
    ...
    disp.extension_add_method(
        'display',
        'xrandr_add_output_mode',
        add_output_mode)
    disp.extension_add_method(
        'display',
        'xrandr_delete_output_mode',
        delete_output_mode)
    disp.extension_add_method('display', 'xrandr_get_crtc_info', get_crtc_info)
    disp.extension_add_method(
        'display',
        'xrandr_set_crtc_config',
        set_crtc_config)
    disp.extension_add_method(
        'display',
        'xrandr_get_crtc_gamma_size',
        get_crtc_gamma_size)
    disp.extension_add_method(
        'display',
        'xrandr_get_crtc_gamma',
        get_crtc_gamma)
    disp.extension_add_method(
        'display',
    ...
    disp.extension_add_method(
        'display',
        'xrandr_set_crtc_transform',
        set_crtc_transform)
    disp.extension_add_method(
        'display',
        'xrandr_get_crtc_transform',
        get_crtc_transform)
    disp.extension_add_method(
        'window',
