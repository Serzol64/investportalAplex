...
class Job(object):
    ...
    def _modify(self, **changes):
        ...
        if 'func' in changes or 'args' in changes or 'kwargs' in changes:
            ...
            if isinstance(args, six.string_types) or not isinstance(
                    args, Iterable):
                raise TypeError('args must be a non-string iterable')
            if isinstance(kwargs, six.string_types) or not isinstance(
                    kwargs, Mapping):
        ...
        if 'misfire_grace_time' in changes:
            ...
            if value is not None and (not isinstance(
                    value, six.integer_types) or value <= 0):
                raise TypeError(
    ...
    def __getstate__(self):
        # Don't allow this Job to be serialized if the function reference could
        # not be determined
    ...
    def __repr__(self):
        return '<Job (id=%s name=%s)>' % (
            repr_escape(self.id), repr_escape(self.name))
