...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    _trigger_plugins = dict((ep.name, ep)
                            for ep in iter_entry_points('apscheduler.triggers'))
    _trigger_classes = {}
    _executor_plugins = dict((ep.name, ep)
                             for ep in iter_entry_points('apscheduler.executors'))
    _executor_classes = {}
    _jobstore_plugins = dict((ep.name, ep)
                             for ep in iter_entry_points('apscheduler.jobstores'))
    _jobstore_classes = {}
    ...
    def add_job(self, func, trigger=None, args=None, kwargs=None, id=None, name=None,
        ...
        # Don't really add jobs to job stores before the scheduler is up and
        # running
        with self._jobstores_lock:
            if self.state == STATE_STOPPED:
    ...
    def modify_job(self, job_id, jobstore=None, **changes):
        ...
        # Wake up the scheduler since the job's next run time may have been
        # changed
    ...
    def resume_job(self, job_id, jobstore=None):
        ...
        with self._jobstores_lock:
            ...
            if next_run_time:
                return self.modify_job(
                    job_id, jobstore, next_run_time=next_run_time)
            else:
    ...
    def remove_job(self, job_id, jobstore=None):
        ...
        with self._jobstores_lock:
            ...
            if self.state == STATE_STOPPED:
                for i, (job, alias, replace_existing) in enumerate(
                        self._pending_jobs):
    ...
    def _configure(self, config):
        # Set general options
        self._logger = maybe_ref(
            config.pop(
                'logger',
                None)) or getLogger('apscheduler.scheduler')
        self.timezone = astimezone(
            config.pop(
                'timezone',
    ...
    def _create_plugin_instance(self, type_, alias, constructor_kwargs):
        ...
        try:
            ...
            if alias in plugin_container:
                ...
                raise LookupError(
                    'No {0} by the name "{1}" was found'.format(
                        type_, alias))
    ...
    def _process_jobs(self):
        ...
        with self._jobstores_lock:
            for jobstore_alias, jobstore in six.iteritems(self._jobstores):
                try:
                    ...
                    # Schedule a wakeup at least in jobstore_retry_interval
                    # seconds
                    self._logger.warning('Error getting due jobs from job store %r: %s',
                                         jobstore_alias, e)
                    retry_wakeup_time = now + \
                        timedelta(seconds=self.jobstore_retry_interval)
        ...
        if self.state == STATE_PAUSED:
            ...
            wait_seconds = min(max(timedelta_seconds(
                next_wakeup_time - now), 0), TIMEOUT_MAX)
