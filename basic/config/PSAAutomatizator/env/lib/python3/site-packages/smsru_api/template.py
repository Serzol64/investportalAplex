...
class ABCSmsRu:
    ...
    def _collect_data(self, numbers: tuple[str], message: str,
        ...
        if ip_address is not None:
            ...
            if not (isinstance(converted_ip, ipaddress.IPv4Address)
                    or isinstance(converted_ip, ipaddress.IPv6Address)):
        ...
        if len(numbers) < 100:
            ...
            raise OutOfPhoneNumbers(
                'Количество номеров телефонов не может быть больше 100 за одн запрос')
        ...
        if timestamp is not None:
            if int(time.time()) - timestamp > 5184000:
                raise OutOfTimestamp(
                    'Задержка сообщения не может быть больше 60 дней')
