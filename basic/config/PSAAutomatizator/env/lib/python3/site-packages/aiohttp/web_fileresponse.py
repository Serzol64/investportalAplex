...
class FileResponse(StreamResponse):
    ...
    async def prepare(self, request: "BaseRequest") -> Optional[AbstractStreamWriter]:
        ...
        if ifmatch is not None and not self._strong_etag_match(
                etag_value, ifmatch):
        ...
        if ifnonematch is not None and self._strong_etag_match(
                etag_value, ifnonematch):
        ...
        if count == 0 or request.method == hdrs.METH_HEAD or self.status in [
                204, 304]:
            return await super().prepare(request)
