...
def content_disposition_header(
    ...
    if not disptype or not (TOKEN > set(disptype)):
        raise ValueError(
            "bad content disposition type {!r}"
            "".format(disptype))
    ...
    if params:
        ...
        for key, val in params.items():
            if not key or not (TOKEN > set(key)):
                raise ValueError(
                    "bad content disposition parameter" " {!r}={!r}".format(
                        key, val)
                )
            if quote_fields:
...
class reify(Generic[_T]):
    ...
    def __get__(self, inst: _TSelf[_T],
                owner: Optional[Type[Any]] = None) -> _T:
        try:
            try:
...
class HeadersMixin:
    ...
    def content_type(self) -> str:
        ...
        raw = self._headers.get(
            hdrs.CONTENT_TYPE)  # type: ignore[attr-defined]
