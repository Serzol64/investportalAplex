...
def _checkForTwoIntOrFloatTuple(arg):
    try:
        if not isinstance(arg[0], (int, float)) or not isinstance(
                arg[1], (int, float)):
            raise PyRectException(
    ...
    for rectOrPoint in rectsOrPoints:
        try:
            ...
            try:
                ...
                raise PyRectException(
                    "argument is not a point or a rect tuple")
...
class Rect(object):
    ...
    def callOnChange(self, oldLeft, oldTop, oldWidth, oldHeight):
        ...
        # Note: This isn't thread safe; the attributes can change between the
        # calling of this function and the code in the function running.
    ...
    def midbottom(self):
        ...
        if self._enableFloat:
            return Point(x=self._left + (self._width / 2.0),
                         y=self._top + self._height)
        else:
            return Point(x=self._left + (self._width // 2),
                         y=self._top + self._height)
    ...
    def midright(self):
        ...
        if self._enableFloat:
            return Point(x=self._left + self._width,
                         y=self._top + (self._height / 2.0))
        else:
            return Point(x=self._left + self._width,
    ...
    def normalize(self):
        ...
        # Note: No need to intify here, since the four attributes should
        # already be ints and no multiplication was done.
    ...
    def __contains__(
        ...
        # Check if value is an (x, y) sequence or a (left, top, width, height)
        # sequence.
        try:
            len(value)
        except BaseException:
...
'''
    ...
    # TODO - Add overloaded operators for + - * / and others once we can
    # determine actual use cases for them.
