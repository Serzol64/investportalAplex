    ...
    def tokenize(self, string):
        ...
        while start < l:
            ...
            if mobj:
                ...
                class_name = next(
                    name for name in self.token_names if groupdict[name])
                token = TokenMeta.getclass(class_name)(groupdict)
                append(token)
                start += len(token.content)
            else:
                raise XMLTokenError(
                    "Unrecognized XML token near %s" % repr(string[:100]))

        return result


tokenize = Tokenizer(
    (Text, Comment, CData, Doctype, XMLDeclaration, Script,
     Style, OpenTag, SelfTag, CloseTag, XMLProcessingInstruction)
).tokenize
