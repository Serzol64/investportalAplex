...
class SimpleInput(object):
    ...
    def render(self, defaults, errors, error_wrapper, stag_end=' />'):
        ...
        if self.name in defaults:
            if(self.tpe == 'file'):
                raise DocError(
                    'Default value for HTML form input of type "file" is not supported')

            attrs['value'] = str(defaults[self.name])
        attrs['name'] = self.name
        lst.append('<input type="%s" %s%s' %
                   (self.tpe, dict_to_attrs(attrs), stag_end))
...
class CheckableInput(object):
    ...
    def render(self, defaults, errors, error_wrapper, stag_end=' />'):
        # type: (Dict[str, Union[List[str], str]], Any, Tuple[str, str], str)
        # -> str
        lst = []
        attrs = dict(self.attrs)
        if self.rank == 0:
...
class Doc(SimpleDoc):
        ...
        def __exit__(self, tpe, value, traceback):
            ...
            if value is None:
                ...
                rendered_select = self.doc.__class__.Select(self.name, self.attrs).render(
                    defaults={},
                    # no defaults for the <select> tag. Defaults are handled by
                    # the <option> tags directly.
                    errors=self.doc.errors,
                    inner_content=inner_content,
                    error_wrapper=self.doc.error_wrapper
                )
    ...
    class OptionTag(object):
        def __init__(self, doc, select, value, attrs):
            # type: (Doc, Doc.SelectTag, str, Dict[str, Union[str, int,
            # float]]) -> None
            self.doc = doc
            self.select = select
            self.attrs = attrs
    ...
    def __init__(self, defaults=None, errors=None,
        ...
        # type: (Optional[Dict[str, str]], Optional[Dict[str, str]], Tuple[str,
        # str], Any, Any) -> None
        """
    ...
    def input(self, *args, **kwargs):
        ...
        self._append(
            checkable_group.input(attrs).render(
                self.defaults,
                self.errors,
                self.error_wrapper,
                self._stag_end))
    ...
    def option(self, *args, **kwargs):
        ...
        if self.current_select:
            ...
            return self.__class__.OptionTag(
                self, self.current_select, value, attrs)
        else:
            raise DocError(
                "No <select> tag opened. Can't put an <option> here.")

    def detached_errors(self, render_function=None):
        # type: (Any) -> None
        self._detached_errors_pos.append(
            (len(self.result), render_function or self.error_dict_to_string))
        self.result.append('')
    ...
    def getvalue(self):
        ...
        for position, render_function in self._detached_errors_pos:
            self.result[position] = render_function(
                dict((name, self.errors[name])
                     for name in self.errors if name not in self._fields)
            )
