    ...
    * Individual values are not stripped of spaces by default.
    ...
    with fid_ctx:
            ...
            for (key, val) in user_filling_values.items():
                if _is_string_like(key):
                    try:
                        ...
                        # We couldn't find it: the name must have been dropped,
                        continue
                # Redefine the key if it's a column number and usecols is
                # defined
                if usecols:
                    try:
                        key = usecols.index(key)
        ...
        for (j, conv) in user_converters.items():
            # If the converter is specified by column names, use the index
            # instead
            ...
            if conv is bytes:
                ...
                # so encode the string again before passing to the user
                # converter
