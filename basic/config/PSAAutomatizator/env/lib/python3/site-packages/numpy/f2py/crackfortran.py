...
def _resolvenameargspattern(line):
    ...
    if m1:
        return m1.group('name'), m1.group(
            'args'), m1.group('result'), m1.group('bind')
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        for l in ll:
            ...
            j = 0
            llen = len(l[1])
            for v in rmbadname([x.strip()
                               for x in markoutercomma(l[0]).split('@,@')]):
                if v[0] == '(':
                    outmess(
                        'analyzeline: implied-DO list "%s" is not supported. Skipping.\n' % v)
...
def _selected_real_kind_func(p, r=0, radix=0):
    ...
    if machine.startswith(
            ('aarch64', 'power', 'ppc', 'riscv', 's390x', 'sparc')):
        if p <= 20:
            return 16
    else:
        if p < 19:
    ...
    for n in svars:
        ...
        if 'attrspec' in vars[n]:
            ...
            if dim and 'dimension' not in vars[n]:
                vars[n]['dimension'] = []
                for d in rmbadname([x.strip()
                                   for x in markoutercomma(dim).split('@,@')]):
                    star = ':' if d == ':' else '*'
