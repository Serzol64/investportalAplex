...
class URL:
    ...
    def __init_subclass__(cls):
        raise TypeError(
            "Inheriting a class {!r} from URL is forbidden".format(cls))
    ...
    def __truediv__(self, name):
        ...
        if name.startswith("/"):
            raise ValueError(
                "Appending path {!r} starting from slash is forbidden".format(
                    name)
            )
        path = self._val.path
        if path == "/":
        ...
        return URL(
            self._val._replace(
                netloc=self._make_netloc(
                    user, password, val.hostname, val.port)
            ),
            encoded=True,
        )
    ...
    def with_password(self, password):
        ...
        return URL(
            self._val._replace(
                netloc=self._make_netloc(
                    val.username, password, val.hostname, val.port)
            ),
            encoded=True,
        )
    ...
    def with_port(self, port):
        ...
        if port is not None and not isinstance(port, int):
            raise TypeError(
                "port should be int or None, got {}".format(
                    type(port)))
        if not self.is_absolute():
            raise ValueError(
        ...
        return URL(
            self._val._replace(
                netloc=self._make_netloc(
                    val.username, val.password, val.hostname, port)
            ),
            encoded=True,
        )
    ...
    def with_path(self, path, *, encoded=False):
        ...
        if len(path) > 0 and path[0] != "/":
            path = "/" + path
        return URL(self._val._replace(
            path=path, query="", fragment=""), encoded=True)
    ...
    def update_query(self, *args, **kwargs):
        ...
        return URL(self._val._replace(
            query=self._get_str_query(query)), encoded=True)
...
def cache_configure(*, idna_encode_size=_MAXCACHE, idna_decode_size=_MAXCACHE):
    ...
    _idna_encode = functools.lru_cache(
        idna_encode_size)(_idna_encode.__wrapped__)
    _idna_decode = functools.lru_cache(
        idna_decode_size)(_idna_decode.__wrapped__)
