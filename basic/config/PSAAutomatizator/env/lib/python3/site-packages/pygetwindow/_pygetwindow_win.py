import ctypes
# We can't use ctypes.wintypes, we must import wintypes this way.
...
def _getAllTitles():
    ...
    # A correction to this code (for enumWindowsProc) is here:
...
def _formatMessage(errorCode):
    ...
    ctypes.windll.kernel32.FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS,
                                          ...
                                          0,  # dwLanguageId
                                          ctypes.cast(
                                              ctypes.byref(lpBuffer), wintypes.LPWSTR),
                                          0,  # nSize
                                          NULL)
...
def _raiseWithLastError():
    ...
    raise PyGetWindowException(
        ...
        (errorCode, _formatMessage(errorCode)))
...
def getActiveWindowTitle():
    ...
    # NOTE - This function isn't threadsafe because it relies on a global
    # variable. I don't use nonlocal because I want this to work on Python 2.
...
def getWindowsAt(x, y):
    ...
    * ``y`` (int, optional): The y position of the window(s)."""
    ...
    for window in getAllWindows():
        if pointInRect(x, y, window.left, window.top,
...
class Win32Window(BaseWindow):
    ...
    def title(self):
        ...
        textLenInCharacters = ctypes.windll.user32.GetWindowTextLengthW(
            self._hWnd)
        ...
        ctypes.windll.user32.GetWindowTextW(
            self._hWnd, stringBuffer, textLenInCharacters + 1)

        # TODO it's ambiguous if an error happened or the title text is just
        # empty. Look into this later.
...
def resolution():
    ...
    """
    return Size(width=ctypes.windll.user32.GetSystemMetrics(
        0), height=ctypes.windll.user32.GetSystemMetrics(1))
