...
def getAllTitles():
    ...
    # Source:
    # https://stackoverflow.com/questions/53237278/obtain-list-of-all-window-titles-on-macos-from-a-python-script/53985082#53985082
    windows = Quartz.CGWindowListCopyWindowInfo(
        Quartz.kCGWindowListExcludeDesktopElements | Quartz.kCGWindowListOptionOnScreenOnly,
        Quartz.kCGNullWindowID)
    return ['%s %s' % (win[Quartz.kCGWindowOwnerName], win.get(
        Quartz.kCGWindowName, '')) for win in windows]
...
def getActiveWindow():
    ...
    for win in windows:
        if win['kCGWindowLayer'] == 0:
            # Temporary. For now, we'll just return the title of the active
            # window.
            return '%s %s' % (win[Quartz.kCGWindowOwnerName],
...
def getWindowsAt(x, y):
    ...
    for win in windows:
        ...
        if pygetwindow.pointInRect(
                x, y, w['X'], w['Y'], w['Width'], w['Height']):
            matches.append('%s %s' % (
                win[Quartz.kCGWindowOwnerName], win.get(Quartz.kCGWindowName, '')))
    return matches


def activate():
    # TEMP - this is not a real api, I'm just using this name to store these
    # notes for now.
    ...
    # Source:
    # https://stackoverflow.com/questions/4905024/is-it-possible-to-bring-window-to-front-without-taking-focus?rq=1
    pass


def getWindowGeometry(title):
    # TEMP - this is not a real api, I'm just using this name to stoe these
    # notes for now.
    windows = Quartz.CGWindowListCopyWindowInfo(
        Quartz.kCGWindowListExcludeDesktopElements | Quartz.kCGWindowListOptionOnScreenOnly,
        Quartz.kCGNullWindowID)
...
def isMinimized():
    ...
    # Use the kCGWindowIsOnscreen to check this. Minimized windows are
    # considered to not be on the screen. (But I'm not sure if there are other
    # situations where a window is "off screen".)

    # I'm not sure how kCGWindowListOptionOnScreenOnly interferes with this.
    pass

# TODO: This class doesn't work yet. I've copied the Win32Window class and
# will make adjustments as needed here.


class MacOSWindow():
    def __init__(self, hWnd):
        # TODO fix this, this is a LP_c_long insead of an int.
        self._hWnd = hWnd

        def _onRead(attrName):
            r = self._getWindowRect(_hWnd)
            # Setting _left directly to skip the onRead.
            self._rect._left = r.left
            # Setting _top directly to skip the onRead.
    ...
    def __str__(self):
        ...
        return '<%s left="%s", top="%s", width="%s", height="%s", title="%s">' % (
            self.__class__.__name__, r.left, r.top, width, height, self.title)
