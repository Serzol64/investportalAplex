...
try:
    ...
    # TODO - Pillow now supports ImageGrab on macOS.
...
if not RUNNING_PYTHON_2:
    # On Python 3, all the isinstance(spam, (str, unicode)) calls will work
    # the same as Python 2.
    unicode = str
...
class PyScreezeException(Exception):
    # This is a generic exception class raised when a PyScreeze-related error
    # happens.
    pass


class ImageNotFoundException(PyScreezeException):
    # This is an exception class raised when the locate functions fail to
    # locate an image.
    pass
...
# TODO - We should consider renaming _locateAll_python to
# _locateAll_pillow, since Pillow is the real dependency.
@requiresPillow
def _locateAll_python(needleImage, haystackImage, grayscale=None,
                      limit=None, region=None, step=1, confidence=None):
    """
    TODO
    """
    ...
    if grayscale:  # if grayscale mode is on, convert the needle and haystack images to grayscale
        ...
        # if not using grayscale, make sure we are comparing RGB images, not
        # RGBA images.
    ...
    needleImageRows = [needleImageData[y *
                                       needleWidth:(y +
                                                    1) *
                                       needleWidth] for y in range(needleHeight)]  # LEFT OFF - check this
    ...
    for y in range(haystackHeight):  # start at the leftmost column
        for matchx in firstFindFunc(
            ...
            for searchy in range(1, needleHeight, step):
                ...
                if needleImageData[searchy * needleWidth:(
                        searchy + 1) * needleWidth] != haystackImageData[haystackStart:haystackStart + needleWidth]:
                    foundMatch = False
                    break
            if foundMatch:
                # Match found, report the x, y, width, height of where the
                # matching region is in haystack.
                numMatchesFound += 1
    ...
    # There was no limit or the limit wasn't reached, but close the file
    # handles anyway.
    if needleFileObj is not None:
        needleFileObj.close()
    if haystackFileObj is not None:
...
def locateOnScreen(image, minSearchTime=0, **kwargs):
    ...
    while True:
        try:
            # the locateAll() function must handle cropping to return accurate
            # coordinates, so don't pass a region here.
...
def locateOnWindow(image, title, **kwargs):
    ...
    if len(matchingWindows) == 0:
        raise PyScreezeException(
            'Could not find a window with %s in the title' %
            (title))
    elif len(matchingWindows) > 1:
        raise PyScreezeException(
            'Found multiple windows with %s in the title: %s' %
            (title, [
                str(win) for win in matchingWindows]))

    win = matchingWindows[0]
    win.activate()
    return locateOnScreen(image, region=(
        win.left, win.top, win.width, win.height), **kwargs)
...
# Knuth-Morris-Pratt search algorithm implementation (to be used by screen
# capture)
...
def center(coords):
    ...
    return Point(coords[0] + int(coords[2] / 2),
                 coords[1] + int(coords[3] / 2))
...
def pixelMatchesColor(x, y, expectedRGBColor, tolerance=0):
    ...
    if len(pix) == 3 or len(expectedRGBColor) == 3:  # RGB mode
        ...
        return (abs(r - exR) <= tolerance) and (abs(g - exG)
                                                <= tolerance) and (abs(b - exB) <= tolerance)
    elif len(pix) == 4 and len(expectedRGBColor) == 4:  # RGBA mode
        r, g, b, a = pix
        exR, exG, exB, exA = expectedRGBColor
        return (abs(r - exR) <= tolerance) and (abs(g - exG) <=
                                                tolerance) and (abs(b - exB) <= tolerance) and (abs(a - exA) <= tolerance)
    else:
...
# TODO - Make this more specific. "Anything else" does not necessarily
# mean "Linux".
else:
