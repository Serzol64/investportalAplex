...
class BaseIOStream(object):
    ...
    def read_bytes(self, num_bytes: int,
                   partial: bool = False) -> Awaitable[bytes]:
    ...
    def read_into(self, buf: bytearray,
                  partial: bool = False) -> Awaitable[int]:
        """Asynchronously read a number of bytes.
    ...
    def _handle_events(
            self, fd: Union[int, ioloop._Selectable], events: int) -> None:
        if self.closed():
    ...
    def _find_read_pos(self) -> Optional[int]:
        ...
        if self._read_bytes is not None and (
            ...
            if self._read_buffer:
                m = self._read_regex.search(
                    self._read_buffer, self._read_buffer_pos)
                if m is not None:
    ...
    def _check_max_bytes(
            self, delimiter: Union[bytes, Pattern], size: int) -> None:
