...
if TYPE_CHECKING:
    ...
    class _WebSocketDelegate(Protocol):
        ...
        def on_message(
                self, message: Union[str, bytes]) -> Optional["Awaitable[None]"]:
            pass
...
class WebSocketProtocol13(WebSocketProtocol):
    ...
    async def accept_connection(self, handler: WebSocketHandler) -> None:
        ...
        try:
            ...
            gen_log.debug(
                "Malformed WebSocket request received",
                exc_info=True)
            self._abort()
            return
    ...
    def _handle_message(self, opcode: int,
                        data: bytes) -> "Optional[Future[None]]":
