...
""".replace(
    ...
    def test_redirect_put_without_body(self):
        # This "without body" edge case is similar to what happens with
        # body_producer.
        response = self.fetch(
    ...
    def test_method_after_redirect(self):
        ...
        for status in [301, 302, 303]:
            ...
            # Other methods are left alone, except for 303 redirect, depending
            # on client
            for method in ["GET", "OPTIONS", "PUT", "DELETE"]:
                resp = self.fetch(
                    url, method=method, allow_nonstandard_methods=True)
...
""".replace(
    ...
    def test_response_times(self):
        ...
        for k, v in response.time_info.items():
            self.assertTrue(
                0 <= v < 1.0, "time_info[%s] out of bounds: %s" %
                (k, v))
