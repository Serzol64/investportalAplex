...
class AuthRedirectTest(WebTestCase):
    ...
    def test_relative_auth_redirect(self):
        response = self.fetch(
            self.get_url("/relative"),
            follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertEqual(
            response.headers["Location"],
            "/login?next=%2Frelative")

    def test_absolute_auth_redirect(self):
        response = self.fetch(
            self.get_url("/absolute"),
            follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue(
            re.match(
    ...
    def get(self, *path_args):
        ...
        for key in self.request.arguments:
            ...
            for bvalue in self.request.arguments[key]:
                if not isinstance(bvalue, bytes):
                    raise Exception(
                        "incorrect type for value: %r" %
                        type(bvalue))
            for svalue in self.get_arguments(key):
                if not isinstance(svalue, unicode_type):
                    raise Exception(
                        "incorrect type for value: %r" %
                        type(svalue))
...
class DecodeArgHandler(RequestHandler):
    def decode_argument(self, value, name=None):
        ...
        if "encoding" in self.request.arguments:
            return value.decode(to_unicode(
                self.request.arguments["encoding"][0]))
        else:
            return value
...
<script src="/analytics.js"/>
    ...
    def test_optional_path(self):
        self.assertEqual(self.fetch_json(
            "/optional_path/foo"), {u"path": u"foo"})
    ...
    def test_get_argument(self):
        ...
        response = self.fetch(
            "/get_argument?foo=bar",
            method="POST",
            body=body)
        self.assertEqual(response.body, b"hello")
        # In plural methods they are merged.
        response = self.fetch(
            "/get_arguments?foo=bar",
            method="POST",
            body=body)
        self.assertEqual(
...
class ExceptionHandlerTest(SimpleHandlerTestCase):
    class Handler(RequestHandler):
        ...
        def log_exception(self, typ, value, tb):
            if isinstance(value, PermissionError):
                app_log.warning(
                    "custom logging for PermissionError: %s",
                    value.args[0])
            else:
...
class BaseStreamingRequestFlowControlTest(object):
    ...
    # Test all the slightly different code paths for fixed, chunked, etc
    # bodies.
...
class XSRFTest(SimpleHandlerTestCase):
    ...
    def test_versioning(self):
        ...
        for cookie_token, body_token in (
                (v1_token, v2_token), (v2_token, v1_token)):
            response = self.fetch(
                "/",
                method="POST",
                body=urllib.parse.urlencode(dict(_xsrf=body_token)),
