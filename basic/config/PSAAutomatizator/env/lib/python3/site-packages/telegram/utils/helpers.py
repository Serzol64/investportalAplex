...
def effective_message_type(
    ...
    if isinstance(entity, Message):
        ...
        raise TypeError(
            f"entity is not Message or Update (got: {type(entity)})")
...
def encode_conversations_to_json(
        conversations: Dict[str, Dict[Tuple, object]]) -> str:
...
def decode_conversations_from_json(
        json_string: str) -> Dict[str, Dict[Tuple, object]]:
...
def decode_user_chat_data_from_json(
        data: str) -> DefaultDict[int, Dict[object, object]]:
...
class DefaultValue(Generic[DVType]):
    ...
    def get_value(obj: Union[OT, 'DefaultValue[OT]']) -> OT:
        ...
        """
        return obj.value if isinstance(
            obj, DefaultValue) else obj  # type: ignore[return-value]
