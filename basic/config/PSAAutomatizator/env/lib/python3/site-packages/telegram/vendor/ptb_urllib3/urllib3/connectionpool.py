...
class HTTPConnectionPool(ConnectionPool, RequestMethods):
    ...
    def _raise_timeout(self, err, url, timeout_value, exc_cls):
        ...
        if isinstance(err, SocketTimeout):
            args.append(
                "%s timed out. (%s timeout=%s)" %
                (desc, desc.lower(), timeout_value))
            raise exc_cls(*args)
        ...
        try:
            try:  # Python 2.7, use buffering of HTTP responses
                ...
                try:
                    ...
                    # otherwise it looks like a programming error was the
                    # cause.
...
class HTTPSConnectionPool(HTTPConnectionPool):
    ...
    def _prepare_proxy(self, conn):
        ...
        if sys.version_info <= (
                2, 6, 4) and not self.proxy_headers:  # Python 2.6.4 and older
