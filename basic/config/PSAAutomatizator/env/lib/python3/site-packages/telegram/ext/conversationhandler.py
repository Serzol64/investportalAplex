...
class ConversationHandler(Handler[Update, CCT]):
    ...
    def __init__(
        ...
        if self.per_chat:
            for handler in all_handlers:
                if isinstance(handler, (InlineQueryHandler,
                              ChosenInlineResultHandler)):
                    warnings.warn(
    ...
    def conversations(self, value: ConversationDict) -> None:
        ...
        for handlers in self.states.values():
            for handler in handlers:
                if isinstance(
                        handler, ConversationHandler) and self.persistence and handler.name:
    ...
    def _schedule_job(
        ...
        if new_state != self.END:
            try:
                # both job_queue & conversation_timeout are checked before
                # calling _schedule_job
    ...
    def check_update(self, update: object) -> CheckUpdateType:  # pylint: disable=R0911
        ...
        if isinstance(state, tuple) and len(
                state) == 2 and isinstance(state[1], Promise):
    ...
    def handle_update(  # type: ignore[override]
        ...
        try:
            new_state = handler.handle_update(
    ...
    def _update_state(self, new_state: object, key: Tuple[int, ...]) -> None:
        if new_state == self.END:
            ...
            with self._conversations_lock:
                ...
                if self.persistent and self.persistence and self.name:
                    self.persistence.update_conversation(
                        self.name, key, (self.conversations.get(
                            key), new_state)
                    )
            ...
            with self._conversations_lock:
                ...
                if self.persistent and self.persistence and self.name:
                    self.persistence.update_conversation(
                        self.name, key, new_state)

    def _trigger_timeout(self, context: CallbackContext,
                         job: 'Job' = None) -> None:
