...
class Bot(TelegramObject):
    ...
    # It's not very clean, but the warnings will be dropped at some point
    # anyway.
    def __setattr__(self, key: str, value: object,
                    ext_bot: bool = False) -> None:
        if issubclass(self.__class__,
                      Bot) and self.__class__ is not Bot and not ext_bot:
    ...
    def _insert_defaults(
        ...
        if not self.defaults:
            data.update((key, DefaultValue.get_value(value))
                        for key, value in data.items())
            return effective_timeout

        # if we have Defaults, we replace all DefaultValue instances with the relevant
        # Defaults value. If there is none, we fall back to the default value
        # of the bot method
    ...
    def _message(
        ...
        if reply_markup is not None:
            if isinstance(reply_markup, ReplyMarkup):
                ...
                # attached to media messages, which aren't json dumped by
                # utils.request
        ...
        if data.get('media') and (data['media'].parse_mode == DEFAULT_NONE):
            if self.defaults:
                data['media'].parse_mode = DefaultValue.get_value(
    ...
    @log
    def get_me(self, timeout: ODVInput[float] = DEFAULT_NONE,
               api_kwargs: JSONDict = None) -> User:
        ...
        # type: ignore[return-value, arg-type]
        self._bot = User.de_json(result, self)
    ...
    def send_message(
        ...
        api_kwargs: JSONDict = None,
        allow_sending_without_reply: ODVInput[bool] = DEFAULT_NONE,
        entities: Union[List['MessageEntity'],
                        Tuple['MessageEntity', ...]] = None,
        protect_content: bool = None,
    ) -> Message:
    ...
    def send_photo(
        ...
        if caption_entities:
            data['caption_entities'] = [me.to_dict()
                                        for me in caption_entities]
    ...
    def send_media_group(
        ...
        for med in data['media']:
            if med.parse_mode == DEFAULT_NONE:
                if self.defaults:
                    med.parse_mode = DefaultValue.get_value(
                        self.defaults.parse_mode)
                else:
                    med.parse_mode = None
        ...
        def _set_defaults(res):
            ...
            if hasattr(res, 'parse_mode') and res.parse_mode == DEFAULT_NONE:
                if self.defaults:
                    ...
                    res.parse_mode = None
            if hasattr(
                    res, 'input_message_content') and res.input_message_content:
                if (
    ...
    def get_chat_members_count(
        ...
        return self.get_chat_member_count(
            chat_id=chat_id, timeout=timeout, api_kwargs=api_kwargs)
    ...
    def answer_shipping_query(  # pylint: disable=C0103
        ...
        if ok:
            ...
            data['shipping_options'] = [option.to_dict()
    ...
    def upload_sticker_file(
        ...
        """
        data: JSONDict = {'user_id': user_id,
                          'png_sticker': parse_file_input(png_sticker)}
    ...
    def send_poll(
        ...
        api_kwargs: JSONDict = None,
        allow_sending_without_reply: ODVInput[bool] = DEFAULT_NONE,
        explanation_entities: Union[List['MessageEntity'],
                                    Tuple['MessageEntity', ...]] = None,
        protect_content: bool = None,
    ) -> Message:
        """
        ...
        if explanation_entities:
            data['explanation_entities'] = [me.to_dict()
                                            for me in explanation_entities]
        if open_period:
    ...
    def get_my_commands(
        ...
        if (scope is None or scope.type ==
                scope.DEFAULT) and language_code is None:
            self._commands = BotCommand.de_list(
                result, self)  # type: ignore[assignment,arg-type]
    ...
    def log_out(self, timeout: ODVInput[float] = DEFAULT_NONE) -> bool:
        ...
        """
        return self._post(
            'logOut', timeout=timeout)  # type: ignore[return-value]
    ...
    def close(self, timeout: ODVInput[float] = DEFAULT_NONE) -> bool:
        ...
        """
        return self._post(
            'close', timeout=timeout)  # type: ignore[return-value]
    ...
    def copy_message(
        ...
        caption: str = None,
        parse_mode: ODVInput[str] = DEFAULT_NONE,
        caption_entities: Union[Tuple['MessageEntity', ...],
