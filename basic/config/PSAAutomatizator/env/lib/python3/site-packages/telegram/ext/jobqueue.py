...
class JobQueue:
    ...
    def _build_args(
            self, job: 'Job') -> List[Union[CallbackContext, 'Bot', 'Job']]:
        if self._dispatcher.use_context:
            return [self._dispatcher.context_types.context.from_job(
                job, self._dispatcher)]
...
class Job:
    ...
    def run(self, dispatcher: 'Dispatcher') -> None:
        ...
        try:
            if dispatcher.use_context:
                self.callback(
                    dispatcher.context_types.context.from_job(
                        self, dispatcher))
            else:
    ...
    def _from_aps_job(cls, job: APSJob, job_queue: JobQueue) -> 'Job':
        ...
        return cls(job.func, context=context, name=job.name,
                   job_queue=job_queue, job=job)
