...
class PicklePersistence(BasePersistence[UD, CD, BD]):
    ...
    def _load_singlefile(self) -> None:
        try:
            ...
            with open(self.filename, "rb") as file:
                ...
                # For backwards compatibility with files not containing bot
                # data
            ...
            raise TypeError(
                f"Something went wrong unpickling {filename}") from exc
    ...
    def flush(self) -> None:
        ...
        if self.single_file:
            ...
            if self.callback_data:
                self._dump_file(
                    f"{self.filename}_callback_data",
                    self.callback_data)
            if self.conversations:
                self._dump_file(
                    f"{self.filename}_conversations",
                    self.conversations)
