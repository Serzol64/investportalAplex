    ...
    * :meth:`flush`
    ...
    def __new__(
        ...
        def update_callback_data_replace_bot(data: CDCData) -> None:
            obj_data, queue = data
            return update_callback_data(
                (instance.replace_bot(obj_data), queue))

        # We want to ignore TGDeprecation warnings so we use obj.__setattr__.
        # Adds to __dict__
        ...
        object.__setattr__(
            instance,
            'get_callback_data',
            get_callback_data_insert_bot)
        object.__setattr__(
            instance,
            'update_user_data',
            update_user_data_replace_bot)
        object.__setattr__(
            instance,
            'update_chat_data',
    ...
    def set_bot(self, bot: Bot) -> None:
        ...
        """
        if self.store_callback_data and not isinstance(
                bot, telegram.ext.extbot.ExtBot):
            raise TypeError(
                'store_callback_data can only be used with telegram.ext.ExtBot.')
    ...
    def _replace_bot(cls, obj: object, memo: Dict[int, object]) -> object:  # pylint: disable=R0911
        ...
        if isinstance(obj, (list, set)):
            ...
            new_iterable = obj.__class__(
                cls._replace_bot(
                    item, memo) for item in temp_iterable)
            memo[obj_id] = new_iterable
            return new_iterable
        if isinstance(obj, (tuple, frozenset)):
            # tuples and frozensets are immutable so we don't need to worry
            # about thread safety
            new_immutable = obj.__class__(
                cls._replace_bot(
                    item, memo) for item in obj)
        ...
        if isinstance(obj, dict):
            ...
            for k, val in temp_dict.items():
                new_obj[cls._replace_bot(
                    k, memo)] = cls._replace_bot(val, memo)
            memo[obj_id] = new_obj
            return new_obj
        try:
    ...
    def _insert_bot(self, obj: object, memo: Dict[int, object]) -> object:  # pylint: disable=R0911
        ...
        if isinstance(obj, dict):
            ...
            new_obj.clear()
            for k, val in temp_dict.items():
                new_obj[self._insert_bot(
                    k, memo)] = self._insert_bot(val, memo)
            memo[obj_id] = new_obj
            return new_obj
        try:
            if hasattr(obj, '__slots__'):
