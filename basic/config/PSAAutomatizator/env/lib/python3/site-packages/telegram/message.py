...
class Message(TelegramObject):
    ...
    def __init__(
        ...
        **_kwargs: Any,
        ...
        if (
            ...
            raise ValueError(
                "Only supply one of `video_chat_ended`/`voice_chat_ended`, not both.")
        if (
        ...
        # those are empty classes, so they don't need a comparison like the
        # others.
    ...
    def de_json(cls, data: Optional[JSONDict],
        ...
        data['voice_chat_started'] = VideoChatStarted.de_json(
            data.get('voice_chat_started'), bot)
        data['voice_chat_ended'] = VideoChatEnded.de_json(
            data.get('voice_chat_ended'), bot)
    ...
    def to_dict(self) -> JSONDict:
        ...
        if self.caption_entities:
            data['caption_entities'] = [e.to_dict()
                                        for e in self.caption_entities]
        if self.new_chat_photo:
            data['new_chat_photo'] = [p.to_dict() for p in self.new_chat_photo]
        if self.new_chat_members:
            data['new_chat_members'] = [u.to_dict()
                                        for u in self.new_chat_members]
