...
class Dispatcher(Generic[CCT, UD, CD, BD]):
    ...
    def __init__(
        ...
        self.context_types = cast(
            ContextTypes[CCT, UD, CD, BD], context_types or ContextTypes())
        ...
        if persistence:
            ...
            if self.persistence.store_callback_data:
                ...
                if persistent_data is not None:
                    if not isinstance(persistent_data, tuple) and len(
                            persistent_data) != 2:
        ...
        # only one instance of Dispatcher, it will be possible to use the
        # `run_async` decorator.
        with self.__singleton_lock:
            if self.__singleton_semaphore.acquire(
                    blocking=False):  # pylint: disable=R1732
    ...
    def __setattr__(self, key: str, value: object) -> None:
        ...
        # it mangled so they don't raise TelegramDeprecationWarning
        # unnecessarily
    ...
    def get_instance(cls) -> 'Dispatcher':
        ...
        if cls.__singleton is not None:
            # type: ignore[return-value] # pylint: disable=not-callable
    ...
    def _pooled(self) -> None:
        ...
        while True:
            ...
            # If unpacking fails, the thread pool is being closed from
            # Updater._join_async_threads
            if not isinstance(promise, Promise):
                self.logger.debug(
                    "Closing run_async thread %s/%d", thr_name, len(
            ...
            # DispatcherHandlerStop nor raised by an error handler. So we can
            # and must handle it
            try:
                self.dispatch_error(
                    promise.update,
    ...
    def run_async(
        ...
        """
        return self._run_async(func, *args, update=update,
                               error_handling=True, **kwargs)
    ...
    def _run_async(
        ...
        **kwargs: object,
        ...
        # TODO: Remove error_handling parameter once we drop the @run_async
        # decorator
        promise = Promise(
            func,
            args,
    ...
    def stop(self) -> None:
        ...
        # Stop all threads in the thread pool by put()ting one non-tuple per
        # thread
        ...
        for i, thr in enumerate(threads):
            self.logger.debug(
                'Waiting for async thread %s/%s to end', i + 1, total)
            thr.join()
    ...
    def process_update(self, update: object) -> None:
        ...
        if handled:
            ...
            # If update was only handled by async handlers, we don't need to
            # update here
            if not handled_only_async:
                self.update_persistence(update=update)

    def add_handler(
            self, handler: Handler[UT, CCT], group: int = DEFAULT_GROUP) -> None:
        """Register a handler.
        ...
        if not isinstance(handler, Handler):
            raise TypeError(
                f'handler is not an instance of {Handler.__name__}')
        ...
        if (
            ...
            handler.conversations = (  # type: ignore[attr-defined]
                self.persistence.get_conversations(
                    handler.name)  # type: ignore[attr-defined]
            )
    ...
    def __update_persistence(self, update: object = None) -> None:
        if self.persistence:
            ...
            # objects will change, when the dict does and we want to loop over
            # chat_ids
    ...
    def add_error_handler(
        ...
        run_async: Union[bool,
                         DefaultValue] = DEFAULT_FALSE,
    ...
    def remove_error_handler(
            self, callback: Callable[[object, CCT], None]) -> None:
        ...
        if self.error_handlers:
            for callback, run_async in self.error_handlers.items(
                ...
                if self.use_context:
                    ...
                    if run_async:
                        self.run_async(
                            callback, update, context, update=update)
                    else:
                        callback(update, context)
                else:
                    if run_async:
                        self.run_async(
                            callback, self.bot, update, error, update=update)
                    else:
