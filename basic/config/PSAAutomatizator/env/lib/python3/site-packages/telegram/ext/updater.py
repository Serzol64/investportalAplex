...
class Updater(Generic[CCT, UD, CD, BD]):
    ...
    def __init__(
        ...
        arbitrary_callback_data: Union[DefaultValue,
                                       bool, int, None] = DEFAULT_FALSE,
    ):
        ...
    ...
    def _init_thread(self, target: Callable, name: str, *
    ...
    def _start_webhook(
        ...
        # Form SSL Context
        # An SSLError is raised if the private key does not match with the
        # certificate
        if use_ssl:
            try:
    ...
    def _bootstrap(
        ...
        def bootstrap_onerr_cb(exc):
            if not isinstance(exc, Unauthorized) and (
                ...
                self.logger.error(
                    'Failed bootstrap phase after %s retries (%s)', retries[0], exc)
                raise exc
    ...
    def stop(self) -> None:
        ...
        with self.__lock:
            if self.running or self.dispatcher.has_running_threads:
                ...
                # Stop the Request instance only if it was created by the
                # Updater
                if self._request:
                    self._request.stop()
    ...
    def _signal_handler(self, signum, frame) -> None:
        ...
        if self.running:
            self.logger.info(
                'Received signal %s (%s), stopping...', signum, get_signal_name(
                    signum)
            )
            if self.persistence:
    ...
    def idle(self, stop_signals: Union[List, Tuple] = (
            SIGINT, SIGTERM, SIGABRT)) -> None:
