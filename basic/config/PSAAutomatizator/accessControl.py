...
if getIp != '127.0.0.1':
    dbConnect = ['localhost', 'zolotaryow_inv', 'pNtJCRTGEZ', 'zolotaryow_inv']
...
def generateCryptoUser(login):
    ...
    contactQuery = (
        'SELECT firstname, country, phone FROM users WHERE login=%s')
    cnlx.execute(contactQuery, (contactQuery))
    ...
    idResponse = hashlib.md5(
        ...
        currentRegion +
        '/' +
        currentName).hexdigest()
    return idResponse


def decryptUserID(hashQuery):
    decryptResponse = generateCryptoUser(hashQuery)
    getResponse = decryptResponse.split('/')

    return {'login': getResponse[0],
            'phone': getResponse[2], 'region': getResponse[3]}
