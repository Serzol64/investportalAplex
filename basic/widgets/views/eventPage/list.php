<?php

/* @var $this yii\web\View */
use yii\helpers\Html;

$description = $eventTable->find(':not(p + table)');
$program = $eventTable->find('p + table');

echo Html::decode($description->outertext);
?>
<div id="eventProgram">
	<?php 
		for($i = 0; $i < count($program); $i++){ 
			$date = $eventTable->find('p', $i);
			$table = $eventTable->find('table', $i);
			
			$mid = 1;
	?>
		
		<span data-type="day"><?php echo $date; ?></span>
								
		<ul data-type="todo">
			<?php for($j = 0; $j < count($table); $j++){ ?>
				<li>
					<span><?php echo Html::decode($eventTable->find('table > tbody tr', $j)->children(0)->outertext); ?></span>
					<span><?php echo Html::decode($eventTable->find('table > tbody tr', $j)->children(1)->outertext); ?></span>
					<?php if($eventTable->find('table > tbody tr', $j)->children(2)->outertext){ ?><a href="#ev<?php echo $mid; ?>" rel="modal:open" class="moreInfo"><img src="data:image/png;base64,"></a><?php } ?>
				</li>
			<?php 
				$mid++;
			} 
			?>						
		</ul>
		<button data-type="incoming" data-eventid="<?php echo $eventId; ?>" class="add-but">Registration in organizator web site</button>
	<?php } ?>
</div>

