<?php

/* @var $this yii\web\View */
use yii\helpers\Html;

$program = (array) $eventTable['programData'];

echo Html::decode($eventTable['description']);

if($program){
?>
<p class="bookmark">Event program</p>
<div id="eventProgram">
	<?php 
		for($i = 0; $i < count($program); $i++){ 
			$table = $program['program'][$i]['feed'];
			$date = $program['program'][$i]['date'];
			$mid = 1;
	?>
		
		<span data-type="day"><?php echo $date; ?></span>
								
		<ul data-type="todo">
			<?php for($j = 0; $j < count($table); $j++){ ?>
				<li>
					<span><?php echo Html::decode($table[$j]['period']); ?></span>
					<span><?php echo Html::decode($table[$j]['title']); ?></span>
					<?php if($table[$j]['content']){ ?><a href="#ev<?php echo $mid; ?>" rel="modal:open" class="moreInfo"><img src="data:image/png;base64,"></a><?php } ?>
				</li>
			<?php 
				$mid++;
			} 
			?>						
		</ul>
	<?php } ?>
		<?php if($organizator){ ?><button data-type="incoming" data-eventid="<?php echo $eventId; ?>" class="add-but">Registration in organizator web site</button><?php } ?>
</div>
<?php } ?>

